...
除了照片判讀主流程，Construction Regulation Knowledge Graph（CRKG，營造法規知識圖譜）也需要支援法規/內規、表單與歷史資料的版本演進。核心原則是：更新時保留舊資料可追溯；新增時由 Graph 協助提出表單或內規建議，再交由人工審核。
...
1. 第一層：原法律/內規修法。
2. 第二層：對應表單欄位或表單版本更新。
3. 第三層：舊表單、舊查驗資料與新表單建立勾稽關係。

此情境的重點是版本映射。舊版法規、舊表單與舊查驗資料不應被覆蓋，而是透過 version_of、supersedes、mapped_to 等關係保留可追溯性，使舊資料仍可與新版表單比較、轉換或彙整。
...
1. 第一層：法律/內規新增法條。
2. 第二層：系統提出表單內容建議，或提出公司內規補充建議。

若新增法條尚未有對應表單，Graph 可根據條文要求、相近既有欄位與歷史查驗資料，建議新的查核欄位、填寫規則、必填條件或內規補充內容。這些建議需經人工審核後才能納入正式 Graph。
...
1. 第一層：法律/內規沒有更新。
2. 第二層：表單新增欄位或新增表單版本。
3. 第三層：建立新表單與既有歷史資料的關聯。

若只是表單新增，Graph 需要記錄新欄位的來源、適用情境、欄位型別與既有歷史資料的對應方式，避免新欄位成為無法追溯的孤立資料。

## 4. 上到下處理流程

### 4.1 流程 A：VLM + 圖譜判讀主流程
...
1. 第一層：法規/內規異動，包含修法、法條新增，或確認法律/內規未更新。
2. 第二層：表單更新或新增，包含表單欄位更新、新版表單、表單內容建議或內規補充建議。
3. 第三層：歷史資料勾稽，將舊表單、舊查驗資料與新表單建立版本映射與可追溯關係。
4. 人工審核：確認建議欄位、內規補充、版本映射是否納入正式 Graph。
5. 更新 Graph：寫入新版法規/內規節點、表單欄位、歷史資料關聯與版本邊。
...
### 5.2 Context-Guided Visual Language Model Perception Module（脈絡引導式 VLM 視覺語言模型觀察模組）
...
### 5.4 CRKG Knowledge Core（營造法規知識圖譜核心）
...
### 5.5 Multi-Category Retriever（多類別圖譜檢索器）
...
### 5.7 Output Generation Layer（輸出產生層）
...
1. Attention Anchor（注意錨點）：由法規/內規、表單與歷史資料產生，告訴 VLM 本張照片要優先檢查什麼。
2. Photo Annotation（照片標記資料）：VLM 依錨點在照片上標出 bbox / polygon，作為可回看的影像證據。
3. VLM 產生的判釋資料（OBS）：VLM 看全局後整理出的判讀資料；描述看見什麼，不直接作最終違規結論。
4. Violation Decision（違規判定資料）：把 OBS、照片標記與法規/表單依據串起來，形成疑似違規、改善建議與填表內容。
